...
if [[ "${3}" && -f "$manifest_file" ]]; then
    echo "Input is a file, not a directory."
    cat "$manifest_file" > generated_manifest.fp-manifest
    ...
    echo "Input is neither a file nor a directory. Please provide a valid input."
    exit 1
fi
